...
TABLE_DIR = os.path.join(base_dir, "tables_output")
KEYWORDS_PATH = os.path.join(base_dir, "all_papers_keywords.json")
OUTPUT_PATH = os.path.join(base_dir, "papers_metric_results.json")
...
def main():
    ...
    for paper_folder in sorted(paper_folders):
        ...
        if paper_key not in all_keywords:
            print(f" Skipping {paper_folder} — not found in all_papers_keywords.json")
            continue
        ...
        if not metrics or not baselines:
            print(f" {paper_folder}: missing metrics or baselines.")
            continue

        print(f"\n Processing {paper_folder}...")
        csv_files = glob.glob(os.path.join(paper_path, "*.csv"))
        if not csv_files:
            print(f" No tables found for {paper_folder}.")
            continue
        ...
        if paper_best:
            ...
            print(f"No matching metrics found for {paper_folder}")
